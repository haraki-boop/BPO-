...
export default function UniversalDashboardPage() {
  ...
  const LOCATION_NAME = 'afs尾西流通センター'; 
  ...
  const [displayTargetMonth, setDisplayTargetMonth] = useState<'current' | 'previous'>('current');
  ...
  // 🌟 【最終入力日基準 ＆ ペース色判定 ＆ 物量の予測達成率 ＆ 今月/先月切り替え 適用版】
        ...
        {['sales', 'manhours', 'volume', 'productivity', 'labor'].includes(activeTab) && (
          <div className={`grid gap-4 md:gap-6 print:grid-cols-2 print:gap-8 ${displayMode !== 'daily' ? 'grid-cols-1 lg:grid-cols-2' : ''}`} style={displayMode === 'daily' ? { gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 450px), 1fr))' } : {}}>
            ...
            {finalSortedMetrics.map((m, i) => {
              ...
              // 🌟 【修正】「先月モード」かつ「生産性タブ」のときだけ！ 先月の目標がなければ今月の目標を引っ張ってくる
              ...
              if (isPrevGlobal && activeTab === 'productivity' && !hasForecastData) {
                if (m.forecast?.some((v: number) => n(v) > 0)) {
                  ...
                  hasForecastData = true; // 黒パネルを表示させるため
                }
              }
              ...
              // 🌟 【追加修正】先月モードで、今月の目標を代替した時は、名前（タイプ）も「目標」などを引っ張る
              ...
              if (isMonthly) {
                ...
                // 🌟 【完全修正】「先月モード」かつ「生産性タブ」で目標が0のときだけ、代替配列から平均を再計算する
              ...
              chartData = chartIndicesForRender.map(idx => {
                ...
                // 🌟 先月モードの時は、グラフのX軸ラベルの日付（月）を1ヶ月戻す
                        ...
                        {(displayMode === 'daily' || displayMode === 'monthly') && (
                          <div className="mt-0">
                            <span className="text-[10px] md:text-[11px] font-bold text-slate-400 block mb-0.5">
                              {isMonthly ? (activeTab === 'volume' ? '実績合計' : (isPrevGlobal ? '月末確定着地' : '月末着地予測')) : (!isAvgMetric ? '合計実績' : '平均実績')}
                            </span>
                          ...
                          return (
                            <div className="flex flex-col gap-1.5 items-end shrink-0">
                              <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl shadow-sm text-right ${isPrevGlobal ? 'bg-slate-100 text-slate-700 border border-slate-200' : 'bg-slate-900 text-white'}`}>
                                {/* 🌟 修正2: 「予算達成率」とハードコーディングされていた箇所を、動的な名前に変更 */}
                      ...
                      <div className={`w-full xl:w-[240px] text-white p-5 rounded-2xl flex flex-col justify-between shrink-0 shadow-inner min-w-0 ${isPrevGlobal ? 'bg-slate-700' : 'bg-slate-900'}`}>
                        <div>
                          <p className={`text-[9px] font-black tracking-widest uppercase mb-3 ${isPrevGlobal ? 'text-slate-300' : 'text-blue-400'}`}>
                            {isMonthly ? (isPrevGlobal ? '月次確定着地' : '月次フォアキャスト確定') : (!isAvgMetric ? '合計確認':'平均確認')}
                          </p>
                          <div className="mb-3">
                            <span className="text-[10px] md:text-[11px] font-bold text-slate-400 block mb-0.5">
                              {isMonthly ? (activeTab === 'volume' ? '実績合計' : (isPrevGlobal ? '月末確定着地' : '月末着地予測')) : (!isAvgMetric ? '合計実績' : '平均実績')}
                            ...
                            <div className={`space-y-2 mt-3 pt-3 border-t ${isPrevGlobal ? 'border-slate-500/50' : 'border-slate-700/50'}`}>
                              <div className="flex justify-between items-baseline">
                                <span className="text-[10px] md:text-xs font-bold text-slate-400 whitespace-nowrap">
                                  {isMonthly ? (activeTab === 'volume' ? `${currentFctType}合計` : `${currentFctType}設定`) : (!isAvgMetric ? `${currentFctType}` : `平均${currentFctType}`)}
                                </span>
